  ...
  return (
    <NavLink
      ...
      className={cn(
        ...
        "text-slate-500 dark:text-slate-300 transition-all duration-200 hover:bg-orange-50 dark:hover:bg-orange-950/20 hover:text-slate-700 dark:hover:text-slate-100"
      )}
      activeClassName="bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-300 font-semibold [&>span.bar]:opacity-100 [&_.ibox]:bg-orange-100 [&_.ibox]:text-orange-500"
    >
      ...
      <span className="ibox flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100/80 dark:bg-slate-800/80 text-slate-400 dark:text-slate-200 transition-all duration-200 group-hover:bg-orange-100 dark:group-hover:bg-orange-900/20 group-hover:text-orange-500 dark:group-hover:text-orange-300">
    ...
    <div>
      <button
        ...
        className={cn(
          ...
          "text-slate-500 dark:text-slate-300 transition-all duration-200 hover:bg-orange-50 dark:hover:bg-orange-950/20 hover:text-slate-700 dark:hover:text-slate-100",
          isOpen && "bg-orange-50/60 dark:bg-orange-500/10 text-slate-700 dark:text-slate-100"
        )}
      >
        <span className={cn(
          ...
          isOpen
            ...
            : "bg-slate-100/80 dark:bg-slate-800/80 text-slate-400 dark:text-slate-200 group-hover:bg-orange-100 dark:group-hover:bg-orange-900/20 group-hover:text-orange-500 dark:group-hover:text-orange-300"
        )}>
          <Icon className="h-4 w-4" />
        </span>
        ...
        <div className="ml-[1.15rem] mt-0.5 border-l-2 border-orange-100 dark:border-orange-900/30 pl-3 py-1 space-y-0.5">
          {submenu.map((sub) => (
            <NavLink
              key={sub.href}
              to={sub.href}
              className="block rounded-lg px-3 py-2 text-xs font-medium text-slate-400 dark:text-slate-300 transition-all duration-200 hover:bg-orange-50 dark:hover:bg-orange-950/20 hover:text-orange-600 dark:hover:text-orange-300"
              activeClassName="bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-300 font-semibold"
            >
...
export function LeftSidebar() {
  ...
  const topMenuItems = isAdmin
    ? topNavigationItems.filter((i) => i.label === "Dashboard" || i.label === "All Courses")
  ...
  return (
    <nav className="relative flex h-full flex-col overflow-y-auto bg-sidebar px-2 py-3 text-foreground [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">

      {/* ── User profile card ── */}
      <div className="relative z-10 mb-4 rounded-2xl p-[1.5px] shadow-sm shadow-orange-100/50 dark:shadow-slate-900/20"
      ...
      {/* bottom fade */}
      <div className="pointer-events-none sticky bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-white dark:from-slate-950 to-transparent" />
    </nav>
  );
}